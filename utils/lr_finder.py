...
class LRFinder(object):
    ...
    def _train_batch(self, batch):
        ...
        #
        # for p in params:
        #     updates[p.replace('lstm.', '')] = grads['lstm_a.' + p]
        # for p in params:
        #     updates[p.replace('lstm.', '')] += grads['lstm_b.' + p]
        #     updates[p.replace('lstm.', '')] = updates[p.replace('lstm.', '')] / 2.0
        #
        # for name, param in self.model.named_parameters():
        #     if param.requires_grad:
        #         if 'lstm_a' in name:
        #             param.grad.data = updates[name.replace('lstm_a.lstm.', '')]
        #         elif 'lstm_b' in name:
        #             param.grad.data = updates[name.replace('lstm_b.lstm.', '')]
